...
{
	 uint16_t currPixyWord, lastPixyWord;
...
int PixyJunk::GetBlocks(int maxBlocks)
{
	 ...
	 for(blockCount=0; blockCount<maxBlocks && blockCount<PIXY_MAXIMUM_ARRAYSIZE;)
	 {
		...
		for (blockIndex = 0, sum = 0; blockIndex < sizeof(Block)/sizeof(uint16_t); blockIndex++)
		{
		   ...
		   *((uint16_t *)block + blockIndex) = pixyWord; //converts block to interger value
		}
		if (checksum == sum)
		   blockCount++;
		else
...
int PixyJunk::AnalyzeTarget()
{
	...
	int prevBlockCount = blockCount;  // save prevous block count in case we get an intermittent null block
	...
	//But if we get two or morein a row then we return no target is found
